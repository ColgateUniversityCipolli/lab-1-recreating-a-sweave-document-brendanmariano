...
\hlkwd{colnames}\hldef{(sleep)}\hlkwb{<-}\hlkwd{c}\hldef{(}\hlsng{"Extra Sleep (hrs)"}\hldef{,}\hlsng{"Drug"}\hldef{,}\hlsng{"Patient"}\hldef{)}
\hldef{sleep.table}\hlkwb{<-}\hlkwd{xtable}\hldef{(sleep,}\hlkwc{label} \hldef{=} \hlsng{"sleep.tab"}\hldef{)} \hlcom{# we can now reference sleep.tab}
\end{alltt}
\end{kframe}
\end{knitrout}
