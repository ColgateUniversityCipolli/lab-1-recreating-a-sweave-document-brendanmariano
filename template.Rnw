...
@
\indent If we want to print this nicely, we can do so using the xtable package \citep{xtable1}, which we can reference using the label (Table \href{VADeaths.tab}{1}).
<<size="scriptsize">>=
library(xtable)
...
\begin{table}[H]
  \begin{center}
    \begin{tabular}{l r r r r r}\hline
      ...
      Zero-Sum (Z) &17.96 &1.00 & 19.48 &\verb|<| 0.0001 & 0.03\\
...
The palmerpenguins package for R \citep{penguins} provides data on adult foraging penguins near Palmer Station, Antarctica. Figure \href{penguinsplot}{2} is too big to fit nicely in our column-based-template above, so I’ve placed it here in the abstract by saving it and presenting it scaled to 0.75.

 <<penguinsplot, size = "scriptsize", results = "hide">>=
